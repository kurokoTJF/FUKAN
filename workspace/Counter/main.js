...
class Player {
    ...
    draw() {
        ...
        if (this.state['buffed']) {
            ...
            c.fillRect(
                ((this.position.x - 1) * canvas.width) / 5,
        ...
        if (this.state["title"]) {
            ...
            if (keys.test.pressed) {
                ...
                _c.processCall = () => {
                    let _t = _c.time
                    if (_c.time == 1)
                        PP.state['darken'] = true
                    if (_c.time == 10)
                        log('darken')
                    if (_c.time == 40)
                        log('player animation')
                    if (_c.time == 100) {
                        log('over')
...
let img = new Image()
let imgLoaded = false
img.src = "https://raw.githubusercontent.com/kurokoTJF/FUKAN/refs/heads/main/Sprites/red/enchant.png"
img.onload = () => {
    imgLoaded = true
}
